...
void main(){
  ...
  pos.xy -=pos.xy;
  pos += length(position)*normalize(position)*smoothstep(uIndex*0.2,1. ,uProgress);  //uProgressで大きく 
  ...
  pos += normalize(rotPos-pos)*length(rotPos-pos)*smoothstep(1.0,2.0,uProgress);    //回転先へ移動
  vec3 rotPos2 = rotate(pos,vec3(1.0,0.0,0.0),-PI/2.);                              //もう90度回転
  pos += normalize(rotPos2-pos)*length(rotPos2-pos)*smoothstep(2.0,3.0,uProgress);  //移動
